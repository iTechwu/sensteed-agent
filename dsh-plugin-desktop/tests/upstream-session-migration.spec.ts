...
it('migrates a released code session through the installed format worker and preserves its V2 log', async () => {
  const root = await mkdtemp(join(tmpdir(), 'sensteed-agent-session-migration-'))
