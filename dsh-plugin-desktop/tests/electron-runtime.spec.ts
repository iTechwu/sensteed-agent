  ...
  return {
    ...
    Menu: {
      buildFromTemplate: vi.fn((template: unknown[]) => {
        ...
        if (first?.label === 'Sensteed-Agent' && Array.isArray(first.submenu)) {
...
describe('Electron desktop runtime', () => {
  beforeEach(() => {
    ...
    updater.filename.mockImplementation((platform: string, version: string) => (
      `Sensteed-Agent-${version}-${platform === 'darwin' ? 'mac.dmg' : 'windows.exe'}`
    ))
  ...
  it('rebuilds ordered effect-scoped tray contributions without replacing native commands', async () => {
    ...
    expect(labels).toEqual([
      'Open Sensteed-Agent', 'Reload Interface', 'Switch to Extended Window', undefined,
  ...
  it('renders contributed radio submenus in their own profile section', async () => {
    ...
    }>).find(item => item.label === 'Sensteed-Agent')
  ...
  it('uses Electron networking and confirmation-gated macOS update handoff', async () => {
    ...
    updater.download.mockResolvedValueOnce('/tmp/Sensteed-Agent-2.1.0-mac.dmg')
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    const release = runtime.schedule(spec)
    await runtime.mountScheduled()
    ...
    expect(updater.download).toHaveBeenCalledWith({
      platform: 'darwin',
      version: '2.1.0',
      destinationPath: '/tmp/Downloads/Sensteed-Agent-2.1.0-mac.dmg',
      request: expect.any(Function),
      signal: controller.signal,
    })
    expect(electron.shell.openPath).toHaveBeenCalledWith('/tmp/Sensteed-Agent-2.1.0-mac.dmg')
    expect(updater.record).toHaveBeenCalledWith('/tmp/sensteed-agent-user-data', {
      platform: 'darwin',
      version: '2.1.0',
      path: '/tmp/Sensteed-Agent-2.1.0-mac.dmg',
    })
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(
      activeWindow,
      expect.objectContaining({
  ...
  it('starts the downloaded Windows installer visibly before requesting orderly exit', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('win32')
    updater.download.mockResolvedValueOnce('C:\\Updates\\Sensteed-Agent-2.1.0-windows.exe')
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: 'C:\\Updates\\Sensteed-Agent-2.1.0-windows.exe',
    })
    ...
    expect(childProcess.spawn).toHaveBeenCalledWith(
      'C:\\Updates\\Sensteed-Agent-2.1.0-windows.exe',
      ['--updated', '--force-run'],
      {
        detached: true,
        stdio: 'ignore',
    ...
    expect(updater.record).toHaveBeenCalledWith('/tmp/sensteed-agent-user-data', {
      platform: 'win32',
      version: '2.1.0',
      path: 'C:\\Updates\\Sensteed-Agent-2.1.0-windows.exe',
    })
  ...
  it.each([
    ...
    const artifact = {
      ...
      version: '2.0.1',
      path: 'C:\\Updates\\Sensteed-Agent-2.0.1-windows.exe',
    }
  ...
  it('rejects a macOS handoff when the operating system cannot open the DMG', async () => {
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: '/tmp/Sensteed-Agent-2.1.0-mac.dmg',
    })
