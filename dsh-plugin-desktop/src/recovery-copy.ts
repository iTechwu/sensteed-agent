...
const COPY: Record<DesktopLocale, DesktopRecoveryCopy> = {
  en: {
    title: 'Sensteed-Agent Recovery Assistant',
    fallbackBody: 'The recovery information could not be read. Quit and start Sensteed-Agent again.',
    ...
    confirmSafeModeBody: 'Sensteed-Agent will create an isolated DSH home without reading the normal DSH data directory. Your normal Profiles and data are not changed. The temporary environment is deleted on the next normal launch after you leave Safe Mode.',
    ...
    configurationFilesBody: 'View or edit the current Profile and shared Harness-home configuration. Restart Sensteed-Agent after making changes.',
    ...
    confirmDisableBody: 'Nothing is deleted. The plugin, its version declaration and its configuration all stay in the current Profile — Sensteed-Agent simply will not load it on the next start. You can enable it again here at any time.',
    confirmDisableAction: 'Disable plugin',
    confirmEnable: 'Enable this plugin?',
    confirmEnableBody: 'Sensteed-Agent will load this plugin again on the next start. If this plugin caused the startup failure, the recovery assistant will open again and you can disable it once more.',
    confirmEnableAction: 'Enable plugin',
    confirmRollback: 'Roll back this configuration?',
    confirmRollbackBody: capturedAt => `This immediately restores the current Profile plus the checkpointed settings.yaml and Harness-home patch captured at ${capturedAt}, including which plugins are enabled. After restarting, Sensteed-Agent will use the rolled-back configuration.`,
    confirmRollbackAction: 'Roll Back',
    uninstalledSuccess: 'The plugin was removed from the current Profile. Restart Sensteed-Agent to use the updated plugin configuration.',
    disabledSuccess: 'The plugin is disabled and still installed. Restart Sensteed-Agent to start without it; nothing was removed from the Profile.',
    enabledSuccess: 'The plugin is enabled again. Restart Sensteed-Agent to load it.',
    rollbackSuccess: slotId => `Rolled back to ${slotId}. Restart Sensteed-Agent to use this configuration; the first healthy start after rollback will preserve all three existing slots.`,
    profileSelectedSuccess: 'This Profile is now selected. Restart Sensteed-Agent to use it.',
  ...
  zh: {
    title: 'Sensteed-Agent 恢复助手',
    fallbackBody: '无法读取恢复信息。请退出并重新启动 Sensteed-Agent。',
    reason: '进入恢复模式的原因',
    ...
    confirmSafeModeBody: 'Sensteed-Agent 将创建一个不读取现有 DSH 数据目录的独立 DSH Home，不会修改原有 Profile 和数据。退出安全模式后，下一次普通启动会自动删除临时环境。',
    ...
    configurationFilesBody: '查看或编辑当前 Profile 与 DSH home 的共享配置。修改后需要重新启动 Sensteed-Agent。',
    ...
    desktopVersion: 'Sensteed-Agent 版本',
    pluginCount: '插件',
    configurationFileCount: '配置文件',
    checkpointSize: '检查点大小',
    unknown: '未知',
    restart: '重启 Sensteed-Agent',
    quit: '退出',
    working: '正在执行恢复操作…',
    back: '返回',
    cancel: '取消',
    ...
    confirmRollbackBody: capturedAt => `将立即恢复 ${capturedAt} 保存的当前 Profile、settings.yaml 与 DSH 数据目录补丁（包括插件的启用状态）；重启后，Sensteed-Agent 将使用检查点中的配置。`,
    confirmRollbackAction: '回滚',
    uninstalledSuccess: '插件已从当前 Profile 中卸载。请重启 Sensteed-Agent 以使用更新后的插件配置。',
    disabledSuccess: '插件已禁用，安装内容仍然保留。重启 Sensteed-Agent 后将不再加载；Profile 中没有任何内容被删除。',
    enabledSuccess: '插件已重新启用。重启 Sensteed-Agent 后将重新加载。',
    rollbackSuccess: slotId => `已回滚到${slotId}。请重启 Sensteed-Agent 以使用该配置；回滚后的第一次健康启动会保留现有三个槽位。`,
    profileSelectedSuccess: '已设为当前 Profile。请重启 Sensteed-Agent 以使用该 Profile。',
